...
public class SeekAlgorithm {
    ...
    public void seekBySCAN(int[] queue, int start) {
        ...
        diskScheduler.setSeekStrategy(new SCANSeekStrategy());
        diskScheduler.executeSeek(queue, start);
    }

    public void seekByFCFS(int[] queue, int start) {
        DiskScheduler diskScheduler = new DiskScheduler();
        diskScheduler.setSeekStrategy(new FCFSSeekStrategy());
        diskScheduler.executeSeek(queue, start);
    }

    public void seekBySSTF(int[] queue, int start) {
        DiskScheduler diskScheduler = new DiskScheduler();
        diskScheduler.setSeekStrategy(new SSTFSeekStrategy());
        diskScheduler.executeSeek(queue, start);
    }

    public void seekByCSCAN(int[] queue, int start) {
        DiskScheduler diskScheduler = new DiskScheduler();
        diskScheduler.setSeekStrategy(new CSCANSeekStrategy());
        diskScheduler.executeSeek(queue, start);
    }

    public void seekByCLOOK(int[] queue, int start) {
        DiskScheduler diskScheduler = new DiskScheduler();
        diskScheduler.setSeekStrategy(new CLOOKSeekStrategy());
        diskScheduler.executeSeek(queue, start);
    }

    // SSTF --> 현재 위치에서 가장 가까운 요소를 큐에서 찾아 처리. 
